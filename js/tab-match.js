...
function initMatch(container) {
  ...
  start.addEventListener('click', () => {
    const data = state.data.map(d => ({ nickname: d.nickname || '익명', meal: Number(d.meal) || 0, nag: Number(d.nag) || 0, pet: Number(d.pet) || 0, honesty: Number(d.honesty) || 0 }));
    if (data.length < 2) { out.innerHTML = `<div class="match-empty">응답이 부족합니다. (2명 이상 필요)</div>`; return }

    start.disabled = true;
    sound.shuffle(); // 매칭 계산 중인 느낌의 효과음

    const nMeal = normalize(data, 'meal'); const nNag = normalize(data, 'nag'); const nPet = normalize(data, 'pet'); const nHon = normalize(data, 'honesty');
    const norms = data.map((d, i) => ({ nickname: d.nickname, vec: [nMeal[i], nNag[i], nPet[i], nHon[i]] }));
    ...
    setTimeout(() => {
      ...
      out.querySelectorAll('.match-card.locked').forEach(card => {
        card.addEventListener('click', () => {
          ...
          const p = top3[idx];
          const a = norms[p.i].nickname || '익명', b = norms[p.j].nickname || '익명';
          const isFirst = idx === 0;
